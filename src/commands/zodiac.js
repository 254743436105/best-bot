module.exports = async (sock, msg, args, jid) => {
  if (!args[0]) return sock.sendMessage(jid, { text: '⚠️ Usage: `!zodiac <DD/MM>`\nExample: `!zodiac 15/08`' });
  const [day, month] = args[0].split('/').map(Number);
  if (!day||!month) return sock.sendMessage(jid, { text: '❌ Use format DD/MM' });
  ...
  await sock.sendMessage(jid, { text: `${sign.e} *Zodiac Sign*\n\nDate: ${day}/${month}\nSign: *${sign.n} ${sign.e}*\nPeriod: ${sign.d}` });
};
