...
module.exports = async (sock, msg, args, jid) => {
  ...
  if (isNaN(num)) {
    return sock.sendMessage(jid, { text: '❌ Invalid amount. Use a number.' });
  }
  ...
  try {
    ...
    if (res.data.result !== 'success') {
      return sock.sendMessage(jid, { text: `❌ Unknown currency: *${fromC}*` });
    }

    const rate = res.data.rates[toC];
    if (!rate) {
      return sock.sendMessage(jid, { text: `❌ Unknown currency: *${toC}*` });
    }
    ...
    await sock.sendMessage(jid, { text: '❌ Could not fetch exchange rates. Try again later.' });
  }
};
