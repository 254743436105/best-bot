module.exports = async (sock, msg, args, jid) => {
  if (args.length < 2) return sock.sendMessage(jid, { text: '⚠️ Usage: `!bmi <weight(kg)> <height(cm)>`\nExample: `!bmi 70 175`' });
  const weight = parseFloat(args[0]), height = parseFloat(args[1]) / 100;
  if (isNaN(weight) || isNaN(height) || height <= 0) return sock.sendMessage(jid, { text: '❌ Invalid values.' });
