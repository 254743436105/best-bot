...
module.exports = async (sock, msg, args, jid) => {
  ...
  if (args[0]==='quit') { const w=game.word; delete games[from]; return sock.sendMessage(jid, { text: `❌ Game over! Word was: *${w}*` }); }
  const letter = (args[0]||'').toLowerCase();
  if (!letter||letter.length!==1||!/[a-z]/.test(letter)) return sock.sendMessage(jid, { text: '❌ Send a single letter.' });
  if (game.guessed.includes(letter)) return sock.sendMessage(jid, { text: `⚠️ Already guessed: *${letter}*` });
  game.guessed.push(letter);
  if (!game.word.includes(letter)) game.wrong++;
  const disp = display(game.word, game.guessed);
  if (!disp.includes('_')) { delete games[from]; return sock.sendMessage(jid, { text: `✅ *You won! 🎉*\nWord: *${game.word}*` }); }
  if (game.wrong >= game.maxWrong) { const w=game.word; delete games[from]; return sock.sendMessage(jid, { text: `💀 *Game Over!*\nWord was: *${w}*` }); }
