module.exports = async (sock, msg, args, jid) => {
  ...
  try {
    ...
    if (!botMember?.admin) {
      return sock.sendMessage(jid, { text: '❌ I need to be a group admin to mute/unmute.' });
    }
    ...
    if (sub === 'on') {
      ...
      await sock.sendMessage(jid, { text: '🔊 Group unmuted! Everyone can send messages now.' });
    } else {
    ...
    await sock.sendMessage(jid, { text: '❌ Failed to update group settings.' });
  }
};
