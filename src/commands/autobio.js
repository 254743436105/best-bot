...
module.exports = async (sock, msg, args, jid) => {
  ...
  if (sub === 'on') {
    ...
    if (!valid.includes(interval)) {
      return sock.sendMessage(jid, { text: `❌ Invalid interval. Use: ${valid.join(', ')}\nExample: \`!autobio on 30s\`` });
    }
    startAutoBio(sock, interval);
    return sock.sendMessage(jid, { text: `✅ *Auto Bio ON!*\nBio updates every *${interval}* with a new quote 💡` });
  }

  if (sub === 'off') {
    stopAutoBio();
    return sock.sendMessage(jid, { text: '🚫 *Auto Bio disabled.*' });
  }

  if (sub === 'now') {
    await updateBio(sock);
    return sock.sendMessage(jid, { text: '✅ Bio updated with a fresh quote!' });
  }
