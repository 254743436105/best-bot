...
module.exports = async (sock, msg, args, jid) => {
  ...
  if (sub === 'off') {
    setEnabled(false);
    return sock.sendMessage(jid, { text: '💾 *Status Saver OFF.* Statuses will no longer be auto-saved.' });
  }

  if (sub === 'list') {
    const saved = getSaved();
    if (!saved.length) return sock.sendMessage(jid, { text: '📭 No statuses saved yet.\n\nEnable with `!savestatus on` first.' });
    const list = saved.slice(-10).reverse().map((s, i) =>
      `${i+1}. 👤 +${s.sender} | ${s.type.toUpperCase()} | ${s.timestamp}`
    );
    return sock.sendMessage(jid, { text: `💾 *Saved Statuses (${saved.length} total)*\n━━━━━━━━━━━━━━━\n${list.join('\n')}` });
  }
