...
module.exports = async (sock, msg, args, jid) => {
  ...
  if (args[0] && ['a', 'b', 'c', 'd'].includes(args[0].toLowerCase())) {
    ...
    if (!trivia) {
      return sock.sendMessage(jid, { text: '❌ No active trivia. Send `!trivia` to start one.' });
    }
  ...
  try {
    ...
    pendingTrivia[from] = {
      ...
      timeout: setTimeout(() => {
        delete pendingTrivia[from];
        sock.sendMessage(jid, { text: `⏰ Time's up! The answer was: *${correctLabel}. ${correct}*` });
      }, 30000),
    };
    ...
    await sock.sendMessage(jid, { text: '❌ Could not fetch trivia. Try again!' });
  }
};
