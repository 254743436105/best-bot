module.exports = async (sock, msg, args, jid) => {
  if (args.length < 2) return sock.sendMessage(jid, { text: '⚠️ Usage: `!temp <value> <unit>`\nUnits: C, F, K\nExample: `!temp 100 C`' });
  const val = parseFloat(args[0]);
  const unit = args[1].toUpperCase();
  if (isNaN(val)) return sock.sendMessage(jid, { text: '❌ Invalid temperature value.' });
  let c, f, k;
  ...
  else return sock.sendMessage(jid, { text: '❌ Use C, F, or K' });
