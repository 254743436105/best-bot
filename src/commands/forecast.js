...
module.exports = async (sock, msg, args, jid) => {
  if (!args.length) {
    return sock.sendMessage(jid, { text: '⚠️ Usage: `!forecast <city>`\nExample: `!forecast Nairobi`' });
  }
  ...
  try {
    ...
    if (!results?.length) return sock.sendMessage(jid, { text: `❌ City not found: *${city}*` });
    ...
    await sock.sendMessage(jid, { text: '❌ Could not fetch forecast. Try again later.' });
  }
};
