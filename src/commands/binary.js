module.exports = async (sock, msg, args, jid) => {
  ...
  if (args[1] === 'd') {
    ...
    if (isNaN(dec)) return sock.sendMessage(jid, { text: '❌ Invalid binary number.' });
    await sock.sendMessage(jid, { text: `💻 *Binary → Decimal*\n\n${args[0]} = *${dec}*` });
  } else {
    const num = parseInt(args[0]);
    if (isNaN(num)) return sock.sendMessage(jid, { text: '❌ Invalid number.' });
    await sock.sendMessage(jid, { text: `💻 *Decimal → Binary*\n\n${num} = *${num.toString(2)}*` });
  }
};
