module.exports = async (sock, msg, args, jid) => {
  if (!args[0]) return sock.sendMessage(jid, { text: '⚠️ Usage: `!age <DD/MM/YYYY>`\nExample: `!age 15/08/2000`' });
  const parts = args[0].split('/');
  if (parts.length !== 3) return sock.sendMessage(jid, { text: '❌ Use format: DD/MM/YYYY' });
  const dob = new Date(`${parts[2]}-${parts[1].padStart(2,'0')}-${parts[0].padStart(2,'0')}`);
  if (isNaN(dob)) return sock.sendMessage(jid, { text: '❌ Invalid date.' });
  const now = new Date();
