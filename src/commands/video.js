...
module.exports = async (sock, msg, args, jid) => {
  ...
  await sock.sendMessage(jid, { text: '⬇️ Downloading video... Please wait.' });
  ...
  try {
    ...
    if (err.message.includes('filesize') || err.message.includes('larger')) {
      return sock.sendMessage(jid, { text: '❌ Video too large (>50MB). Try a shorter video.' });
    }
    await sock.sendMessage(jid, { text: '❌ Could not download video. Make sure the URL is valid and public.' });
  }
};
